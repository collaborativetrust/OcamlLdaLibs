...
let rec iteri f v = 
  let rec offsetiteri k = function
    Empty -> ()
  | Node(l, cl, d, r, _, _) ->
      offsetiteri k l; f (k + cl) d; offsetiteri (k + cl + 1) r
  in offsetiteri 0 v
...
let rangeiteri f i j v = 
  let rec offsetrangeiteri k i j = function
      Empty -> ()
    | Node(l, cl, d, r, cr, _) ->
      if i < j then begin 
	if i < cl && j > 0 then offsetrangeiteri k i j l; 
	if i <= cl && j > cl then f (k + cl) d;
	if j > cl + 1 && i <= cl + cr + 1 
	then offsetrangeiteri (k + cl + 1) (i - cl - 1) (j - cl - 1) r
      end
  in offsetrangeiteri 0 i j v 

let revrangeiteri f i j v = 
  let rec offsetrevrangeiteri k i j = function
      Empty -> ()
    | Node(l, cl, d, r, cr, _) ->
      if i < j then begin 
	if j > cl + 1 && i <= cl + cr + 1 
	then offsetrevrangeiteri (k + cl + 1) (i - cl - 1) (j - cl - 1) r;
	if i <= cl && j > cl then f (k + cl) d;
	if i < cl && j > 0 then offsetrevrangeiteri k i j l
      end
  in offsetrevrangeiteri 0 i j v 
...
let foldi f v accu =
  let rec offsetfoldi k v accu = 
    match v with
      Empty -> accu
    | Node(l, cl, d, r, _, _) ->
	offsetfoldi (k + cl + 1) r (f (k + cl) d (offsetfoldi k l accu))
  in offsetfoldi 0 v accu
	
let rangefoldi f i j v accu = 
  let rec offsetrangefoldi k i j v accu = 
    match v with 
      Empty -> accu
    | Node (l, cl, d, r, cr, _) -> 
	if i >= j then accu
  ...
  in offsetrangefoldi 0 i j v accu 

let revfoldi f v accu =
  let rec offsetrevfoldi k v accu = 
    match v with
      Empty -> accu
    | Node(l, cl, d, r, _, _) ->
	offsetrevfoldi k l (f (k + cl) d (offsetrevfoldi (k + cl + 1) r accu))
  in offsetrevfoldi 0 v accu

let revrangefoldi f i j v accu = 
  let rec offsetrevrangefoldi k i j v accu = 
    match v with 
      Empty -> accu
    | Node (l, cl, d, r, cr, _) -> 
	if i >= j then accu
  ...
  in offsetrevrangefoldi 0 i j v accu;; 

let rec exists p = function 
    Empty -> false
  | Node (l, _, d, r, _, _) -> 
      exists p l || p d || exists p r;;

let find p n v = 
  (* k is how many elements there are to the left; 
     i is where to start the search *)
  let rec offset_find k i = function 
    Empty -> None 
  | Node (l, cl, d, r, _, _) -> begin 
      match offset_find k i l with 
	Some x -> Some x 
      | None -> begin 
	  if k <= i && p d 
	  then Some (cl + k, d) 
	  else offset_find (cl + k + 1) (i - cl - 1) r
	end
    end
  in offset_find 0 n v 

let rec filter p = function 
    Empty -> Empty 
  | Node (l, _, d, r, _, _) -> 
      let lf = filter p l in 
      let lr = filter p r in 
      if p d then recbal lf d lr else concat lf lr;;
...
if false then begin 
  ...
  let print_l (b: unit) (d: int) : unit = 
    begin print_int d; print_string " " end in 
  let print_r (c: unit) (b: unit) : unit = () in 
  visit_in () print_l print_r v;
  print_string "\n"; 
