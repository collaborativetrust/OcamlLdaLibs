(* Mapmin: implementation of maps where the minimum element is accessible. 
...
(* Derived from map.ml by Luca de Alfaro 
   This module also allows accessing the minimum and maximum elements
   of a map. *)

(** Association tables over ordered types with access to minimum 
...
module type S =
  sig
    ...
    (** [min_elt m] returns the pair [k, el] consisting of a key [k] 
	and element [el] that consitute the minimum element in the
	mapping. *)

    val max_elt : 'a t -> key * 'a
    (** [max_elt m] returns the pair [k, el] consisting of a key [k] 
	and element [el] that consitute the maximum element in the
	mapping. *)
