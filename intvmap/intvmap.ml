...
module Make(Ord: OrderedType) = struct
    ...
    let bracket x m = 
      let rec intv_find x = function
          Empty -> (None, None) 
	| Node(l, v, d, r, _) ->
	    begin 
              let c = Ord.compare x v in
              if c = 0 then (Some (v, d), Some (v, d))
              else begin
		if c < 0 then 
		  begin 
		    (* searches on the left *)
		    let (fl, fr) = intv_find x l in 
		    match fr with 
		      None -> (fl, Some (v, d))
		    | Some _ -> (fl, fr)
		  end
      ...
      in
      let (fl, fr) = intv_find x m in 
      match (fl, fr) with 
	(Some bl, Some br) -> (bl, br)
      | (Some (v, d), None) -> ((v, d), (v, d))
      | (None, Some (v, d)) -> ((v, d), (v, d))
